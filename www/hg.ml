...
let onload _ =
  ...
  in
  let ontextchange _ =
    let l = Lexing.from_string (Js.to_string (entry##.value)) in
    ...
    let _ = Place.elastic g in
    let view = Geometry.scale_box 1.1 (Graph.bbox g) in
  ...
  entry##.onchange := Html.handler ontextchange;
  Js._false
